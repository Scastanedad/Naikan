...
class Boton(pygame.sprite.Sprite):
    ...
    def configurar_filtro(self, nuevo_filtro):
        ...
        super_temp_base.fill(
            (
                self.color_base_original[0],
                self.color_base_original[1],
                self.color_base_original[2],
                255,
            )
        )
        self.base_color = Filtros.aplicar_filtro(super_temp_base, nuevo_filtro).get_at(
            (0, 0)
        )

        super_temp_hover = pygame.Surface((1, 1), pygame.SRCALPHA)
        super_temp_hover.fill(
            (
                self.color_hover_original[0],
                self.color_hover_original[1],
                self.color_hover_original[2],
                255,
            )
        )
        self.hovering_color = Filtros.aplicar_filtro(
            super_temp_hover, nuevo_filtro
        ).get_at((0, 0))
    ...
    def update(self, position):
        hover = self.rect.collidepoint(position) or getattr(
            self, "seleccionado_por_teclado", False
        )
